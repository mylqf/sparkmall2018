...
import com.lqf.sparkmall2018.common.model.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.lqf.sparkmall2018.common.util.{ConfigUtil}
...
object MockerOffline {
  ...
  def insertHive(sparkSession: SparkSession, tableName: String, userVisitActionDF: DataFrame) = {

    sparkSession.sql("use "+ConfigUtil.getValueByKey("hive.database"))
    sparkSession.sql("drop table if exists "+tableName)
    userVisitActionDF.write.saveAsTable(tableName)
  ...
  def mockUserInfo()={
    ...
    for(i<- 1 to userNum){
      val user=UserInfo(i,"user_"+i,"name_"+i,RandomNum(ageFrom,ageTo),professionRandomOpt.getRandomOpt(),genderRandomOpt.getRandomOpt())
      rows+=user
    }
    rows.toList
